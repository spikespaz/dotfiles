...
let
  ...
  _pathType' = d: n: let ls = builtins.readDir d; in imply (ls ? ${n}) ls.${n};
  ...
  _getIndexAttrName = path: fName: type:
    ...
    in imply isValid (if hasDefault then
      fName
    else if isNix then
      (rsplit "." fName).l
    else
  ...
  evalIndices = { pass ? { }, expr, }:
    ...
    else if builtins.isPath expr && _isImportable expr then
      evalIndices {
        pass = pass';
        expr = import expr;
      }
...
in {
  inherit traceM traceValM updates joinNixosModules joinHomeModules
    mkUnfreeOverlay deepMergeAttrs imply implyDefault indicesOf split lsplit
    rsplit evalIndices mkModuleIndex toTOMLFile toTOML;
}
